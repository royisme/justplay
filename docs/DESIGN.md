...
> [!WARNING]
> **Status Update (Jan 2026)**: This design document generally refers to the previous Python/FastAPI iteration where the entire plot connected graph was generated by LLM upfront.
> The **current implementation** (v2, TS/Cloudflare) uses a **Hybrid Architecture**:
>
> - **Structure**: **Ink (.ink)** scripts defines the rigid plot structure, branching logic, and state updates.
> - **Execution**: **inkjs** runtime executes the logic deterministically.
> - **Prose**: AI Agents (Writer/DM) generate the narrative text _at runtime_ or _during compilation_.
>
> Please refer to `server/runtime/ink-runner.ts` and `README.md` for the single source of truth regarding the current architecture. The sections below are kept for historical context or future aspirational features (like dynamic story map generation).
